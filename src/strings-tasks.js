...
function removeFirstOccurrences(str, value) {
  ...
  const str1 = str.substring(0, str.indexOf(value));
  const str2 = str.substring(str.indexOf(value) + value.length);
...
function removeLastOccurrences(str, value) {
  ...
  const str1 = str.substring(0, str.lastIndexOf(value));
  const str2 = str.substring(str.lastIndexOf(value) + value.length);
  return str1.concat(str2);
}
...
function encodeToRot13(str) {
  ...
  function encoding(char) {
    let n = 26;

    if (char === char.toLowerCase()) n = alphabet.length;

    return alphabet[
      alphabet.indexOf(char) + 13 > n - 1
        ? alphabet.indexOf(char) - 13
        : alphabet.indexOf(char) + 13
    ];
  }

  return str.replace(/[a-z]/gi, (char) => encoding(char));
}
